...
func registerWebRouter(router *mux.Router) error {
	...
	webBrowserRouter.Methods(http.MethodPost).Path("/backup/retrieve/plan").HandlerFunc(httpTraceHdrs(web.RetrieveBackupPlan))
	webBrowserRouter.Methods(http.MethodPost).Path("/snapshot/add/job").HandlerFunc(httpTraceHdrs(web.SnapshotAddBackupJob))
	//webBrowserRouter.Methods(http.MethodPost).Path("/snapshot/backup/volume").HandlerFunc(httpTraceHdrs(web.SnapshotSendOfflineDealsVolume))
	webBrowserRouter.Methods(http.MethodPost).Path("/backup/retrieve/volume").HandlerFunc(httpTraceHdrs(web.RetrieveOfflineDealsVolume))
	webBrowserRouter.Methods(http.MethodPost).Path("/snapshot/backup/retrieve/volume").HandlerFunc(httpTraceHdrs(web.SnapshotRetrieveOfflineDealsVolume))
	webBrowserRouter.Methods(http.MethodPost).Path("/snapshot/rebuild/add/job").HandlerFunc(httpTraceHdrs(web.SnapshotAddRebuildJob))
	webBrowserRouter.Methods(http.MethodPost).Path("/rebuild/retrieve/volume").HandlerFunc(httpTraceHdrs(web.RetrieveRebuildVolume))
	webBrowserRouter.Methods(http.MethodPost).Path("/snapshot/rebuild/retrieve/volume").HandlerFunc(httpTraceHdrs(web.SnapshotRetrieveRebuildVolume))
